...
def setup_auth_file(pull_secret_path):
    ...
    if os.path.exists(pull_secret_path):
        ...
        try:
            # We use Python's built-in JSON module instead of 'jq' to avoid external dependencies
...
def setup_local_mirror_registry(registry_fqdn):
    ...
    # Because we already set REGISTRY_AUTH_FILE in setup_auth_file(), podman login will safely append to it.
...
def main():
    ...
    setup_auth_file(args.pull_secret)
